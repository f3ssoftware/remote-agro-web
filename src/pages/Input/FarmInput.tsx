...
export function FarmInput() {
  ...
  const find = () => {
    setProducts(
      input?.inputs?.filter((product: Product) => {
        if (
          product!
            .product!.name?.toUpperCase()
            .includes(findTerm.toUpperCase()) ||
          product!.product!.class?.includes(findTerm.toUpperCase())
        ) {
          return product
        }
        return null
  ...
  return (
    <div className="app-container">
        ...
        <Card.Title>
          <Row>
            ...
            <Col md={2} sm={2}>
              <Button
                className="inputs-btn"
                onClick={() => setShowSeedsTreatment(true)}
              >
                Tratar Sementes
              </Button>
            </Col>
          ...
          <div
            style={{ maxHeight: '60vh', overflowY: 'scroll', marginTop: '5%' }}
          >
            <DataTable
              value={products}
            ...
            >
              ...
              ></Column>
              <Column field="product.class" header="Classe"></Column>
              <Column
                field="quantityInDecimal"
                header="Estoque Atual"
                body={(rowData: any) => {
                  return (
                    <span>
                      {rowData.quantityInDecimal} {rowData.measure_unit}
                      <FontAwesomeIcon
                        icon={faEye}
                        style={{ marginLeft:'4%', color: '#000AFF', cursor: 'pointer' }}
                        onClick={() => {
                          setShowHistoryModal(true)
                          setHistorySelectedProduct(rowData)
                        }}
                      ></FontAwesomeIcon>
                    </span>
                  )
              ...
              <Column
                ...
                body={(rowData: any) => {
                  const costPerUnit =
                    rowData.total_price / 100 / rowData.quantityInDecimal
                  return costPerUnit.toLocaleString('pt-BR', {
              ...
              <Column
                ...
                header="Custo Total"
                body={(rowData: any) => {
                  return (rowData.total_price / 100).toLocaleString('pt-BR', {
                    maximumFractionDigits: 2,
                    style: 'currency',
                    currency: 'BRL',
                    useGrouping: true,
              ...
              <tbody style={{ backgroundColor: '#fff', color: '#000' }}>
                {products.map((input: Product, index) => {
                  return (
                    <tr key={index}>
                      <td>
                        {input.product?.class === 'SEMENTE'
                          ? input?.product?.name + ' ' + input?.treatment
                          : input?.product?.name}
                      </td>
                      ...
                      <td>
                        {(input.total_price! / 100).toLocaleString('pt-BR', {
                          maximumFractionDigits: 2,
                          style: 'currency',
                          currency: 'BRL',
                          useGrouping: true,
      ...
      ></HistoryModal>
      <EntranceModal
        show={showEntranceModal}
        handleClose={() => setShowEntranceModal(false)}
      ></EntranceModal>
      <WithdrawalModal
      ...
      ></WithdrawalModal>
      <SeedsTreatment
        show={showSeedsTreatment}
        handleClose={() => setShowSeedsTreatment(false)}
      ></SeedsTreatment>
    </div>
  )
}
